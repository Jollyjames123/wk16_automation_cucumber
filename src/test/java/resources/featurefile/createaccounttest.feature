Feature: Creating new account Test
  ...
  to create account with invalid credentials
    ...
    And clicks on create an account tab
    And fill all mandatory fields
      | James | DD12** | abc$%12456 | 123 Abc Str%%t | Lo$%^on | Florida | 123ssa45 | 0123abcd67890 |
    And click on register
    Then verify that invalid information will give an error message "There are 5 errors"
